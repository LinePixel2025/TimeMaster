...
- 应用管理页是独立 QWidget `src/ui/app_manage_page.{h,cpp}`（左侧应用列表 + 右侧属性面板 + 下方组别管理区），由 `SettingsDialog` 直接 `addWidget` 进 `m_stack`。改动即时落库（与旧的应用管理一致），不参与「保存/取消」事务；任一改动发 `appsChanged()`，`SettingsDialog` 转发为 `settingsChanged()`，从而触发追踪层 `reloadSettings()`。页面自带 `pageStyle()`（只作用于本页控件），不要再往 `settingsStyle()` 里塞新页面的样式。
...
- 管理界面数据 `getManagedApps()` 刻意**不**应用 `min_record_threshold` 与屏蔽过滤（否则用户无法解除屏蔽或看到极短会话的应用），与统计查询的行为不同，改这两处时不要顺手统一。
