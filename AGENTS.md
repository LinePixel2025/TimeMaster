...
Windows 桌面时间追踪应用。项目使用 C++17、Qt 6 Widgets/Sql/Network/Svg、CMake 和 Ninja，当前版本为 5.6.5。
...
- `installer.iss` 使用 Inno Setup 6，从 `dist\TimeMaster` 复制文件并生成 `dist\TimeMaster-Setup-5.6.5.exe`。
...
src/
  ...
  ui/                       MainWindow、主题管理器、托盘管理器和仪表盘卡片
                            ...
                            AppManagePage（设置「应用管理」页，独立 QWidget）
                            group_icons.h：组别 emoji 图标定义（预设固定图标 + 色板，数据库与 UI 共用）
  ...
  update/                   UpdateChecker：定期检查 GitHub Releases 新版本（更新提醒）
...
tests/                      test_database、test_app_management、test_exporter、test_lineweb_pusher、test_cloud_sync、test_ai_client、test_reminder_scheduler、test_weekly_report、test_daily_report、test_window_tracker、test_trend_chart_layout、test_trend_card_geometry、test_dashboard_layout、test_period_distribution_layout、test_rank_layout、test_theme_manager、test_update_checker
...
- 应用管理页是独立 QWidget `src/ui/app_manage_page.{h,cpp}`，由 `SettingsDialog` 直接 `addWidget` 进 `m_stack`，构造需注入 `AiClient*`（供「AI 识别」，MainWindow→SettingsDialog→AppManagePage 传递）。布局为左右两列：应用列表是本页主角（hero 卡片，约 3/5 宽、占满整页高度），右列上方为应用详情属性面板、下方为紧凑组别管理卡片（列表固定高约 140px，操作说明放 tooltip 不再占行）。改动即时落库（与旧的应用管理一致），不参与「保存/取消」事务；任一改动发 `appsChanged()`，`SettingsDialog` 转发为 `settingsChanged()`，从而触发追踪层 `reloadSettings()`。页面自带 `pageStyle()`（只作用于本页控件），不要再往 `settingsStyle()` 里塞新页面的样式；`pageStyle` 的透明背景必须写成 `#appManagePage { background: transparent; }`（objectName 在构造时设置），绝不能退回无选择器的 `QWidget { background: transparent; }`——它会级联进以本页为父的 QMessageBox/QToolTip，把弹窗背景掏空成黑色。应用列表与组别列表均为自绘 delegate；列表按累计时长降序；组行显示 emoji 图标 + 名称 + 预设芯片 + 成员数，「所属组别」下拉同样带图标。以 PID 形式记录的应用（追踪层取不到 exe 路径时进程键为 `pid_<数字>`）不逐行显示，统一折叠为列表末尾的「未识别应用」行（「?」色块 + 个数芯片 + 合计时长），点击弹出非模态清单（`ensurePidDialog`），点选条目后右侧详情联动；详情面板对 PID 应用额外显示「AI 识别」按钮：`DatabaseManager::getRecentWindowTitles` 取窗口标题 → `AiClient::identifyApp` 推断应用名，结果经确认后才 `setAppAlias` 落库。列表默认隐藏累计时长不足 2 分钟（含无记录占位进程）的条目，由「隐藏低时长应用」开关（设置键 `app_manage_hide_placeholder`，默认开）控制，仅影响列表展示、不改数据；折叠行不受该开关影响，但组别筛选激活时整体隐藏。点选组别会把应用列表筛选为该组组员（与搜索取交集），工具栏「只看 …」芯片退出；退出判定必须读 `selectedItems()`——`clearSelection()` 后 `currentItem()` 仍可能指向旧行。详情面板的当前应用以 `m_currentKey` 为唯一来源（可能指向弹窗内的 PID 应用，不在列表行上），新建/编辑组别用页内 `GroupEditDialog`（名称 + emoji 色板单选），色板与预设映射在 `src/ui/group_icons.h`。
- `RankCard` 支持「应用排行 / 组别排行」双类别：`setData(appData, groupData)` 且 groupData 非空时才显示切换按钮，`refresh()` 为仅应用的兼容入口。卡片内部缓存两份数据，10 秒刷新不会冲掉用户当前所选类别。组别行用 `RankLayout::Item::isGroup` 标记，绘制首字色块而非应用图标。
- 报告（日报/周报）复用 `ReportHtml::buildAppRank()` 渲染组别排行，未配置组别时不输出该卡片；不要在 `report_html_builder` 里新增样式副本。
...
- AI 智能模块：`AiClient` 通过 OpenAI 兼容 `POST {endpoint}/chat/completions` 生成报告（`Authorization: Bearer <key>`，解析 `choices[0].message.content`），支持 daily/weekly 两个周期。报告文本与锚点日期缓存于 `ai_report_{daily,weekly}_{text,date}` 设置，跨天/跨周后由 `AiReportCard` 判定过期。主页卡片点击「生成分析」→ `MainWindow::aiReportRequested` → `AiClient::generateReport`，结果经 `reportReady/reportFailed` 回填卡片并联动日报网页；`refreshData` 不触发 AI 请求。`identifyApp(windowTitles, tag)` 供应用管理页识别 PID 应用：`pid_` 兜底标题会被过滤，结果经 `identifyReady/identifyFailed` 按 tag（进程键）回传，返回清洗后的应用名（`sanitizeAppName` 去引号/句读/首行截断，「未知」按失败处理），**不写缓存、不自动落库**，由页面确认后写别名。
...
- 设置界面管理以下设置：`tracking_enabled`、`poll_interval`、`idle_threshold`、`min_tracking_seconds`、`min_record_threshold`、`auto_start`、`theme`、`accent_color`、`daily_goal`,`lineweb_enabled`、`lineweb_endpoint`、`lineweb_token`、`lineweb_interval`、`lineweb_last_push`、`lineweb_last_fetch`、`lineweb_pending_push`、`ai_enabled`、`ai_api_endpoint`、`ai_api_key`、`ai_model`、`ai_report_daily_text`、`ai_report_daily_date`、`ai_report_weekly_text`、`ai_report_weekly_date`、`reminder_enabled`、`reminder_times`、`reminder_last_fired`、`reminder_interval_enabled`、`reminder_interval_minutes`、`weekly_report_enabled`、`weekly_report_day`、`weekly_report_time`、`weekly_report_last_generated`、`weekly_report_path`、`trend_display_format`、`trend_heatmap_period`、`update_last_check`、`update_latest_version`、`update_latest_published`、`update_latest_notes`、`update_latest_url`、`update_latest_download`、`update_notified_version`、`app_groups_seeded`（预设组别播种哨兵，值为 `1`）。新增设置沿用 `settings` 表的 key/value 形式并提供默认值。
- 更新检查：`UpdateChecker` 请求 `GET https://api.github.com/repos/LinePixel2025/TimeMaster/releases/latest`（Accept `application/vnd.github+json`，15 秒超时），解析最新 Release 的版本号、更新内容、安装包直链（assets 中第一个 `.exe`）。启动 60 秒后首次检查、此后每 24 小时一次；成功时把结果缓存到 `update_latest_*` 设置与 `update_last_check`，失败静默。自动检查发现新版本且未提醒过（`update_notified_version` 去重，跨重启生效）时先经托盘气泡提醒再弹 `UpdateDialog`（更新内容 + 「立即更新」打开下载页/「稍后」）；设置「关于」页「更新」卡片提供【检查更新】手动入口（`checkNow(true)` 只发 `checkFinished` 不触发自动弹窗，避免重复），检查完成后弹窗显示更新内容/「已是最新」/失败原因。版本比较 `isNewerVersion` 逐段数值比较（任意段非纯数字视为无更新）；`UpdateDialog` 为 markdown→轻量 HTML 展示，无程序内自动下载/安装，更新通过浏览器下载安装包完成。
- 数据库构造时自动创建/迁移 `sessions`、`settings`、`ignored_apps`、`app_aliases`、`app_groups`、`app_group_members`、`app_merges` 表，迁移使用 `CREATE ... IF NOT EXISTS`，没有版本号迁移框架。
- 应用身份一律以 `process_key`（`ProcessIdentity::normalizeKey`）为准：`app_name` 是写入时固化的自由文本，同一进程可能有多个变体；屏蔽、合并、组别三张表都只存归一化键。合并与组别是**不改写 `sessions`** 的映射/归属关系，解除即可复原。
- `app_groups` 在 `migrate()` 中按 `app_groups_seeded` 哨兵键只播种一次预设组别（开发效率/设计创作/社交通讯/影音娱乐/办公文档/游戏），哨兵**先于**插入写入，因此用户删掉某个预设后重启不会被塞回；预设组别可改名可删除，`builtin` 列仅标记来源。**不会自动把任何应用加入组别。**
- `app_groups.icon` 存组别 emoji 图标：预设播种时带固定图标；老库 `ALTER` 补列后在每次 `migrate()` 里只回填 `icon` 为空的行（预设名取固定值、其余按名称哈希从色板取），用户已挑选的图标永不被覆盖。`getGroups()` 返回的 `icon` 已做兜底解析恒非空；`addGroup(name, icon)` 不传图标时自动分配回退值；`setGroupIcon(id, "")` 允许写空（下次启动回填）。图标定义集中在 `src/ui/group_icons.h`（纯 Qt Core 头，数据库层与 UI 层共用，勿在别处复制色板）。
- `getGroupRank()` 在「尚无任何应用被归入组别」（`app_group_members` 为空）时返回空，由调用方隐藏组别维度——否则整份数据会坍缩成一个与应用排行重复的「未分组」桶。已有成员时，未分组应用才自动汇入「未分组」。
- 管理界面数据 `getManagedApps()` 刻意**不**应用 `min_record_threshold` 与屏蔽过滤（否则用户无法解除屏蔽或看到极短会话的应用），与统计查询的行为不同，改这两处时不要顺手统一。返回按累计时长降序、同值按显示名升序。「不足 2 分钟不显示」只发生在应用管理页的列表展示层（`filterApps`，可开关恢复），不要下推到数据库查询。`getRecentWindowTitles(processKey, limit)` 供「AI 识别」：按标题去重、最近出现时间降序。
...
- 开关类控件一律用 `ToggleSwitch`（`src/ui/toggle_switch.h`，继承 QCheckBox、自绘胶囊轨道 + 滑动圆钮 + 120ms 缓动），不要用 `QCheckBox::indicator` QSS 伪装开关（画不出圆钮）；成员声明仍可为 `QCheckBox*`。界面按钮的线性图标统一在 `src/ui/settings_icons.h`：24px 视觉网格、1.5px 描边、圆头圆角、亮暗主题取色、HiDPI 2x 渲染；新增图标只在 `Kind` 枚举与 `detail::drawIcon` 扩展，不要引入图标资源文件。
